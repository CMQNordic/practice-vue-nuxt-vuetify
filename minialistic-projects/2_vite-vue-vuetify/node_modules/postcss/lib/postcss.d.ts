...
import Node, { Position, Source, ChildNode, NodeErrorOptions, NodeProps, ChildProps, AnyNode } from 'postcss/lib/node.js';
...
type DocumentProcessor = (document: Document, helper: Helpers) => Promise<void> | void;
...
type CommentProcessor = (comment: Comment, helper: Helpers) => Promise<void> | void;
...
interface Processors {
	 ...
	 * Will be called again on node or children changes.
	 */
	DeclarationExit?: DeclarationProcessor | { [prop: string]: DeclarationProcessor };
...
export interface Parser<RootNode = Root | Document> {
	(css: string | { toString(): string }, opts?: Pick<ProcessOptions, 'map' | 'from'>): RootNode;
}
